...
export class UsersService {
  ...
  public getViews(): Observable<any> {
    return this.apiService.get('users/views');
  }

  /**
   * Get views card infos of current user
   *
   * @method GET
   * @return - API response
   * @memberOf {UsersService}
   */
  getViewsCard(): Observable<any> {
    return this.apiService.get('users/views/card');
  }

  /**
   * Update the position of the current user's views
   *
   * @method PUT
  ...
  updatePosition(body: any): Observable<any> {
    return this.apiService.put('users/views/position', body);
  }
